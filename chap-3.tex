...
First, the input data of the LSTM is calculated which should be preprocessed in different ways depending on the model as is showed in Figures \ref{fig:stateless_input} and \label{fig:stateful_input}. This is done before training by making use of a GPU that is listed in Table \ref{tab:CPU}. The inputs that are used are discussed in Section \ref{s:Inputs}. Three different models will be discussed further from which the first two are stateless models and the third a stateful model. Every model will predict only one half hour consumption at a time as was proposed as good methodology in \cite{ANNRNN}. Also, each model makes use of early stopping on a validation set to avoid overfitting. The models can also have multiple LSTM layers on top of each other. The   The specific parameters and layout that is chosen for each of the three models is discussed in Section \ref{s:Parameter search}.
...
\begin{figure}[h]
	...
	\caption{Model2 - stateless model with as input a subserie of N time steps. }
	\label{fig:model2}
\end{figure}

In this model additionally to the output of the last LSTM block also the previous outputs of the LSTM blocks are inputs to the dense layer. 

\subsection{Model 3: Stateful}
\begin{figure}[h]
	\centering
	\includegraphics[width=1.2\textwidth]{model3.png}
	\caption{Model 3 - Statful model that connects single LSTM blocks. }
	\label{fig:model3}
\end{figure}
