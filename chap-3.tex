...
The advantage of using a stateful model in comparison to a stateless one, is that during training and prediction more data can be presented to the hidden and memory states before they are reset. Therefore, the capability of the LSTM of long term remembering important features of the data, is more exploited. However, a practical disadvantage of using a stateful model is that Keras expects that the batch size chosen during training equals the one during prediction. Each prediction uses one sample of input matrix $ \bm{X} $ to output one prediction value. This means that the batch size during training is forced to be equal to one. It was tried to circumvent this by first training on a chosen batch size and then transferring the learned model weights to a new model with batch size one. It was found however the second model only gave back the same predictions as model one when the


Further, a stateful model expects that the training and validation sets are divisible by the batch size. In order to meet these conditions samples of the input matrix are thrown away. 


Methods that are looked into for  circumventing these practical issues were 


can be tried to be circumvented by training a model and 


- setting over of weights --> only works when same model

- working directly in tensorflow
...
The  MAE  error metric is used to evaluate the prediction performance on the validation set for the models during the parameter search in Section \ref{s:Parameter search}. During training the MSE is used in the objective function which is used to derive weight updates.
