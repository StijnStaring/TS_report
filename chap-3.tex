...
It was found in \cite{Greff2017}, that the learning rate is a very important parameter that has to be tuned correctly to enhance the model performance. Therefore, during phase three a range of learning rates is tried for the best model after inspection of phase one and two. The learning rate that leads to the the smallest MAE on the validation set is selected. The different learning rates that are tried are $ 10^{-2} $, $ 5\times10^{-3} $, $ 2\times10^{-3} $, $ 10^{-3} $, $ 10^{-4} $ and $ 10^{-5} $. The selection of multiple learning rates of size of magnitude $ 10^{-3} $, is conform the parameter search in \cite{Shi2018}.\\
