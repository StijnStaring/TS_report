...
\subsubsection{Overfitting avoidance in Keras}
 ...
 Early stopping, dropout and recurrent dropout and  regularization (kernel, bais, activity and recurrent) is used to help to stop overfitting. 

\subsubsection{Metrics to evaluate the models}


\subsection{LSTM}\label{s:LSTM_implementation}
The ``long short-term memory model'' 
- model --> first lstm layers and then a dense layer.
Use of flatten layer and return sequence equal to true --> \cite{Kong2019}
...
\subsection{GRU}\label{s:GRU_implementation}
