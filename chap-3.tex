...
The calculations executed in this chapter are performed on a virtual machine using the Microsoft Azure services. This was possible because Microsoft provides students with $ \$ 100 $ of start credit for Microsoft Azure. Table \ref{tab:CPU} shows the different features of the machines used during this thesis. 
...
\begin{itemize}
	...
	\item Pip installs dependencies in a serial loop while Conda does effort to fulfil all package dependencies simultaneously using a SAT-solver.  
\end{itemize}

However, it is sometimes still necessary to use Pip because this is the only option to install a package. Using an isolated environment on a local machine makes it easy to transfer this environment automatically to a YAML file that is send to the virtual machine.\\

Finally it is noted that while performing the calculation a warning was seen telling that the TensorFlow binary was not compiled to support CPU instructions as SSE4.1 SSE4.2 AVX AVX2 AVX512F FMA. This means that the binaries are originating from a different machine and that the CPU used to run the calculations on the moment is not able to use all its features. This warning was not fixed, but is given as a possible suggestion to the reader to 

this was also the case for the local machine used

Your CPU supports instructions that this TensorFlow binary was not compiled to use: SSE4.1 SSE4.2 AVX AVX2 AVX512F FMA


- tensorflow was not build from source and is not configured to use CPU not configured for using binary files

 that is used during development of the python scrips 
