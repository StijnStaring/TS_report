...
Normalization is necessary because while absolute consumption differs, relative patterns of human behaviour are more similar \cite{Lago2020}. The patterns in the human behaviour is what a forecasting model is trying to predict and normalization contributes by avoiding the disturbance of different magnitudes in which this human pattern may occur. Every individual household time-serie is normalized based on its yearly consumption. The advantage of using the yearly consumption to normalize in comparison of the minimum and maximum values, is the robustness against measurements out shooters and every smart meter has a total consumption of one at the end of the year.
...
Finally, the average is taken over all the remaining $251$ time-series to obtain a single signal. This is done to investigate the dependence of the smart meters on seasonality, temperature, weekends and holidays. At the end of this chapter a baseline forecasting will be introduced that will be used as null-hypothesis in chapter \ref{cha:Evaluating results}to assess if the developed models lead to an improvement.
...
In order to look at the impact of a holiday all the holidays all the holidays of the English and welsh holiday calendar are identified for the year $ 2017 $. For each of the $ 8 $ holidays a corresponding business day is selected with an as close as possible average temperature of the day. This is done to remove the temperature dependency. The resulting average holiday and business day with similar temperature is given in Figure \ref{fig:bvsh}. It can be seen that the consumption of a holiday is reasonably similar to business day.  \textbf{This is maybe not so important and can be left out...}
...
%\subsection{Identification of driving attributes} --> can possibly do regression 

\section{Baseline model}
The baseline 24 hour forecasting model that is chosen is just the current day.
